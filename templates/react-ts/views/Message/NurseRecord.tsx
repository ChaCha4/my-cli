...
const NurseRecord: (props: NurseRecordProps) => React.JSX.Element = (props) => {
    ...
    const nurseTableColumns = [{
        ...
        dataIndex: 'number',
        key: 'number',
        render: (text: string) => <span className='text-[#929EAB]'>{text}</span>
    }, {
        title: '内容',
        dataIndex: 'content',
        key: 'content',
        ...
        <div className='w-[calc(30%-10px)] md:w-full'>
            {!isMobile && (
                <span className='flex items-center justify-between mb-[10px]'>
                    ...
                    <Drawer maskClosable={false} title={<span className='text-2xl'>记录护理项目</span>} onClose={() => {
                        form.resetFields()
                        setCheckedList([])
                        setRecordOpen(false)
                    }} open={recordOpen}>
                <Form form={form} onFinish={handleRecordForm}>
                    ...
                    <Form.Item className='flex justify-around w-full'>
                        <Button color="primary"  variant="outlined" className='w-[8rem] h-[2.4rem] mr-[10px]' onClick={() => {
                            form.resetFields()
                            setCheckedList([])
                            setRecordOpen(false)
                        }}>
                        ...
                        <Button type="primary" htmlType="submit" className='w-[8rem] h-[2.4rem]'>
                            保存
                        </Button>
                    </Form.Item>
                </Form>
            ...
            <div ref={nurseRef} className='bg-[#fff] py-[25px] px-[25px] md:w-[94%] md:rounded-[10px] md:my-[10px] md:mx-auto md:py-[1rem] md:px-[1rem]'>
                ...
                <Table id='nurseTable' rowKey="number" columns={nurseTableColumns} dataSource={dataSource} pagination={false}/>
            </div>
        </div>
    )
}
